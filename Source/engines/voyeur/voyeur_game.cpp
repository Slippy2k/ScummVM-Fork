...
void VoyeurEngine::doPiracy() {
	...
	for (int idx = 0, yp = 33; idx < 10; ++idx) {
		...
		if (VoyeurEngine::getLanguage() == Common::EN_ANY) {
			_screen->_vPort->drawText(PIRACY_MESSAGE[idx]);
		}
		else if (VoyeurEngine::getLanguage() == Common::DE_DEU) {
			_screen->_vPort->drawText(gPIRACY_MESSAGE[idx]);
		}	
...
Common::String VoyeurEngine::getDayName() {
	...
	switch (_voy->_transitionId) {
		...
		case 3:
		case 4: return inSAT;
		case 17:return inMON;
		default:return inSUN;
...
Common::String VoyeurEngine::getTimeOfDay() {
	...
	if (VoyeurEngine::getLanguage() == Common::EN_ANY) {
		...
		inAM = gAM;
		inPM = gPM;
		return Common::String::format("%d:%02d%s", _voy->_isAM ? _gameHour : _gameHour+12, _gameMinute, _voy->_isAM ? inAM : inPM);
	}
	return Common::String::format("%d:%02d%s", _gameHour, _gameMinute, _voy->_isAM ? inAM : inPM);
...
int VoyeurEngine::doComputerText(int maxLen) {
	...
	if (_voy->_RTVNum > _voy->_computerTimeMax && maxLen == 9999) {
		...
		if (VoyeurEngine::getLanguage() == Common::EN_ANY) {
			...
			_screen->_vPort->drawText(gEND_OF_MESSAGE);
		}	
		...
		if (VoyeurEngine::getLanguage() == Common::EN_ANY) {
			...
			_screen->_vPort->drawText(gSTART_OF_MESSAGE);
		}		
