...
Common::String EventsManager::getEvidString(int eventIndex) {
	...
	case Common::DE_DEU:
		return Common::String::format("%03d %.2d:%.2d %s %s", eventIndex + 1, e._isAM ? e._hour : e._hour + 12,e._minute, e._isAM ? gAM : gPM, gEVENT_TYPE_STRINGS[e._type - 1]);
	case Common::EN_ANY:
	default:
		return Common::String::format("%03d %.2d:%.2d %s %s", eventIndex + 1, e._hour, e._minute, e._isAM ? AM : PM, EVENT_TYPE_STRINGS[e._type - 1]);
	}
}
