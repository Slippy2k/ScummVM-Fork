...
void ImageConverter::convert8bit(::Graphics::Surface &trueColor,
	...
	// For now, we only support 8bit->16bit->24Bit conversion	
	if (trueColor.format.bytesPerPixel == 2) {
	...
	if (trueColor.format.bytesPerPixel == 4) {
		assert(trueColor.format.bytesPerPixel == 4);
		uint32 *dst = (uint32 *)trueColor.getPixels();
		const byte *src = (const byte *)paletted.getPixels();

		for (int32 y = 0; y < paletted.h; y++)
			for (int32 x = 0; x < paletted.w; x++, dst++, src++)
				*dst = (uint32)getColor(*src, palette);
	}
}
...
uint32 ImageConverter::getColoralpha(uint8 a, uint8 r, uint8 g, uint8 b) const {
	return _format.ARGBToColor(a, r, g, b);
}
