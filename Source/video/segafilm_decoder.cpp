...
bool SegaFILMDecoder::loadStream(Common::SeekableReadStream *stream) {
	...
	_header.audioFrequency = _stream->readUint16BE();
	...
	if (audioSampleSize != 0) {
		...
		addTrack(new SegaFILMAudioTrack(&_header, getSoundType()));
	}
...
//SegaFILMDecoder::SegaFILMAudioTrack::SegaFILMAudioTrack(uint audioFrequency, uint audioFlags, getSoundType()) {
//	_audioFlags = audioFlags;
//	_audioStream = Audio::makeQueuingAudioStream(audioFrequency, audioFlags & Audio::FLAG_STEREO);
//}

SegaFILMDecoder::SegaFILMAudioTrack::SegaFILMAudioTrack(const SFAHeader *header, Audio::Mixer::SoundType soundType) :
	AudioTrack(soundType) {
	_audioStream = Audio::makeQueuingAudioStream(header->audioFrequency, false);
}
